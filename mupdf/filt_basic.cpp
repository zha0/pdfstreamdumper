	...
	while (p < ep)
	{
		...
		else if (c == '~')
		{
			...
			case 0:
				break;
			case 1:
				printf("partial final byte in a85d");
			case 2:
	...
	while (p < ep)
	{
		...
		if (state->n == 0)
		{
			...
			if (state->run > 128)
			{
				...
				if (state->c < 0)
					printf("premature end of data in run length decode");
			}
		}
...
read_aesd(fz_stream *stm, unsigned char *buf, int len)
{
	...
	while (state->ivcount < 16)
	{
		int c = fz_read_byte(state->chain);
		if (c < 0)
			printf("premature end in aes filter");
	...
	while (p < ep)
	{
		...
		if (n == 0)
			...
			printf("partial block in aes filter");
		...
		if (fz_is_eof(state->chain))
		{
			int pad = state->bp[15];
			if (pad < 1 || pad > 16)
				printf("aes padding out of range: %d", pad);
			state->wp -= pad;
		}
